...
function sanitizeConfig(input, base) {
  ...
  return {
    ...
    birthday: sanitizeBirthday(source.birthday, base.birthday),
  }
}

/** 生日只收 `MM-DD`（或空串 = 没设）；月/日越界或格式不对就退回原值。 */
function sanitizeBirthday(value, fallback) {
...
/**
 * 今天到底演哪一出：`off` 关掉；显式指定就用指定的（生日那种没填日期就退回自动）；
 * `auto` 时生日优先，然后按日期窗口。
 * @param config - 已校验的配置
...
function activeFestival(config, date) {
  ...
  if (config.festival !== 'auto') {
    return config.festival === 'birthday' && config.birthday === '' ? autoFestival(date) : config.festival
  }
...
async function configRoute(req, res) {
  ...
  try {
    ...
    writeConfig(next)
    sendJson(res, 200, { ok: true, config: next, festival: activeFestival(next, new Date()) })
  } catch (error) {
    sendJson(res, 400, { ok: false, error: error instanceof Error ? error.message : String(error) })
  }
}
