...
/**
 * 预览页：同一份启动页样式 + boot.js，挂在一个假主界面上。
 ...
 */
function previewPage(splashCss) {
  return '<!doctype html>\n<html lang="zh-CN">\n<head>\n'
    + '<meta charset="utf-8">\n'
  ...
  apply(ctx) {
    ...
    ctx.effect(() => {
      ...
      const disposers = [
        ...
        route(BASE + '/preview', (req, res) => send(res, 200, 'text/html; charset=utf-8', Buffer.from(previewPage(splashCss)))),
      ]
