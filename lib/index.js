...
/**
 * 主界面 hero 的默认配置（页面侧拿不到 /config 时也用这一份兜底）。
 * `headline` 是要逐字打出来的问候语，`speed` 是每个字的间隔毫秒。
 */
 ...
 * 这样无论页面传来什么（空、缺字段、乱类型、超长文案），落盘的一定是完整可用的一份。
...
/**
 * hero 配置：GET 读一份，POST 存一份（body 是 JSON，可以是局部字段；`{"reset": true}` 回默认）。
...
async function configRoute(req, res) {
  ...
  try {
    ...
    if (input !== null && typeof input === 'object' && input.reset === true) {
      ...
      writeConfig(fresh)
      sendJson(res, 200, { ok: true, config: fresh })
      return
    }
    const next = sanitizeConfig({ ...readConfig(), ...input }, DEFAULT_CONFIG)
    writeConfig(next)
 ...
 */
function inject(html, splashCss, wallCss, heroCss) {
...
export default {
  ...
  apply(ctx) {
    ...
    ctx.effect(() => {
      ...
      disposers.push(ctx.webServer.tapIndex((html) => inject(html, splashCss, wallCss, heroCss)))
