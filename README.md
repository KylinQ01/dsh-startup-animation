...
| 加载好了 | 背景被"镜头收住"：从推近状态收回到 1:1 并拉掉虚化（看得见的那一下），舞台同时撤走、整层淡出，主界面从 1.035 归位淡入 —— 一起做完 |
...
**过场是"一条 700ms 的时间轴"，三处要成对改**：`boot.css` 里 `#dshs.is-out` 的
`opacity .7s`、`html.dshs-boot-out #root` 的 transition，都要和 `boot.js` 的 `OUT_MS = 700`
一致，否则主界面会早于或晚于启动页出现。

**别给过场加 `transition-delay`，也别加 `will-change`**：主界面这时正在首次渲染，
