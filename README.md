...
**保留**一次性的入场：进入主界面那道光带、模块组装（行程自动收到 35%）、启动页淡出。
理由是这条设置针对的是大位移、视差、持续旋转这类容易引发不适的动效，而不是"入场不许有动画"——
以前一刀切全关，结果在关掉了 Windows 动画的机器上**什么都看不见**（`prefers-reduced-motion` 读的就是那个开关）。

> 想让入场也彻底静音：在 `boot.css` 的 `prefers-reduced-motion` 块里给 `.dshs-beam` 加 `animation: none`，
> 再把 `boot.js` 里 `assemble()` 开头的 `if (calm) return` 加回去。
>
